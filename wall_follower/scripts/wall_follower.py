...
class WallFollowerPID:
    ...
    def get_error(self, scan, desired_distance):
        ...
        a = self.get_range(scan, -math.radians(45)) #-45도의 lidar값을 측정
        b = self.get_range(scan, -math.radians(90)) # -90의 Lida값을 측정 
    ...
    def scan_callback(self, scan):
        ...
        if min_front < FRONT_THRESHOLD:
            ...
            self.pub.publish(twist)
            rospy.loginfo("TURN | front: %.2f left: %.2f right: %.2f", min_front, left, right)
        else:
            # 벽 따라간다 (PID)
